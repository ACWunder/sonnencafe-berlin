...
// Fetches all cafés/restaurants for the covered districts and saves to public/cafes-cache.json

import { writeFileSync, mkdirSync } from "fs";

const OVERPASS_URL = "https://overpass-api.de/api/interpreter";
const BBOX = "52.4546381,13.3362902,52.5585856,13.4721073";
...
// ── District polygons (matches overpass.ts) ───────────────────────────────────
...
function guessDistrict(lat, lng) {
  for (const { name, ring } of DISTRICT_POLYGONS) {
    if (pointInPolygon(lat, lng, ring)) return name;
  }
...
function streetFacingPoint(nodeIds, nodeCoords, centLat, centLon) {
  ...
  for (let i = 0; i < pts.length; i++) {
    ...
    if (dist > bestDist) { bestDist = dist; best = { lat: midLat, lon: midLon }; }
  }
  return best;
}

console.log("Fetching cafés from Overpass...");
...
for (const el of data.elements) {
  ...
  cafes.push({ id: key, name, lat, lng: lon, address: addr || undefined, district, tags, amenity: tags.amenity });
}

mkdirSync("public", { recursive: true });
writeFileSync("public/cafes-cache.json", JSON.stringify({ cafes, generated: new Date().toISOString() }));
console.log(`✓ Saved ${cafes.length} cafés to public/cafes-cache.json`);
